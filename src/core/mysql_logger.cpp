...
bool MySqlLogger::GetSubrunDurationSec(uint32_t run_number,
                                       uint32_t subrun_number,
  ...
  std::string cell;
  std::ostringstream sql;
  sql << "SELECT UNIX_TIMESTAMP(end_time) - UNIX_TIMESTAMP(start_time) FROM `"
      << daq_defaults::kMySqlRunLogTable << "` "
      << "WHERE run=" << static_cast<unsigned long long>(run_number)
      << " AND subrun=" << static_cast<unsigned long long>(subrun_number)
      << " LIMIT 1";
  if (!QueryFirstCell(sql.str(), cell, error_text)) {
  ...
  try {
    std::size_t pos = 0;
    out_duration_sec = std::stod(cell, &pos);
    if (pos != cell.size()) {
      error_text = "invalid subrun duration response from DB: " + cell;
      return false;
    }
    out_found = true;
    error_text.clear();
    ...
    error_text = "failed to parse subrun duration response from DB: " + cell;
    return false;
  }
}
